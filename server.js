...
app.post('/carga-inicial', requireAuth, requireRoles(['admin']), async (req, res) => {
    try {
        ...
        const [pronResult] = await connection.execute(`
            INSERT INTO prontuarios (id, paciente_id, profissional_id, data_atendimento, queixa_principal, historia_doenca, historico_doencas, exames_fisicos, diagnostico, tratamento, evolucao, data_cadastro, tipo_atendimento) VALUES
            (1, 1, 1, '2026-01-15', 'Dor lombar crônica', 'Paciente refere dor na região lombar há 6 meses', 'Hipertensão controlada', 'Exame físico normal', 'Hérnia de disco L4-L5', 'Fisioterapia + AINE', 'Paciente apresentando melhora da dor com fisioterapia', NOW(), 'consulta'),
            (2, 2, 2, '2026-01-20', 'Limitação de movimento no ombro direito', 'Após queda da própria altura há 2 meses', 'Mãe com artrite reumatoide', 'Limitação de movimentos', 'Lesão do manguito rotador', 'Fisioterapia intensiva', 'Recuperação lenta mas progressiva', NOW(), 'avaliacao'),
            (3, 3, 3, '2026-01-10', 'Dor no joelho esquerdo', 'Dor progressiva ao caminhar', 'Diabetes tipo 2', 'Exame físico normal', 'Artrose grau II', 'Perda de peso + Fisioterapia', 'Paciente aderindo ao tratamento', NOW(), 'retorno'),
            (4, 4, 4, '2026-01-25', 'Palpitações', 'Episódios de taquicardia ao esforço', 'Pai com cardiopatia isquêmica', 'Exame cardíaco normal', 'Arritmia benigna', 'Beta-bloqueador se necessário', 'Exames normais, manter observação', NOW(), 'consulta'),
            (5, 5, 1, '2026-02-01', 'Dor abdominal', 'Dor epigástrica pós-prandial', 'Asma leve', 'Exame abdominal normal', 'Gastrite leve', 'Omeprazol + dieta', 'Sintomas melhoraram com medicação', NOW(), 'consulta')
        `);
